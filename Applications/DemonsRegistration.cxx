...
struct arguments
{
  ...
  friend std::ostream& operator<< (std::ostream& o, const arguments& args)
    ...
    return o
      ...
      <<"  Displacement field sigma: "<<args.sigmaDef<<std::endl
...
void parseOpts (int argc, char **argv, struct arguments & args)
{
  ...
  //todo
  command.SetOption("DisplacementFieldSigma","s",false,"Smoothing sigma for the deformation field (pixel units). Setting it value below 0.5 means no smoothing will be performed");
  command.SetOptionLongTag("DisplacementFieldSigma","def-field-sigma");
  command.AddOptionField("DisplacementFieldSigma","floatval",MetaCommand::FLOAT,true,"1.5");
...
void DemonsRegistrationFunction( arguments args )
{
  ...
  if ( args.verbosity > 0 )
    {
    typedef itk::DisplacementFieldJacobianDeterminantFilter
       <DisplacementFieldType, PixelType> JacobianFilterType;
